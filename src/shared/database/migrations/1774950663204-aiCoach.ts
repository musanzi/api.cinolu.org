...
export class AiCoach1774950663204 implements MigrationInterface {
    name = 'AiCoach1774950663204'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`ai_coach\` (\`id\` uuid NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`name\` varchar(255) NOT NULL, \`profile\` text NOT NULL, \`role\` text NOT NULL, \`expected_outputs\` text NOT NULL, \`model\` varchar(255) NOT NULL DEFAULT 'llama3.2:3b', \`status\` varchar(255) NOT NULL DEFAULT 'active', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`coach_conversation\` (\`id\` uuid NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`status\` varchar(255) NOT NULL DEFAULT 'active', \`coachId\` uuid NULL, \`ventureId\` uuid NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`coach_message\` (\`id\` uuid NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`role\` varchar(255) NOT NULL, \`output_type\` varchar(255) NOT NULL DEFAULT 'USER_MESSAGE', \`content\` text NOT NULL, \`payload\` text NOT NULL, \`conversationId\` uuid NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
